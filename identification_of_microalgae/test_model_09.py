...
# 按照图片命名自然顺序排序
# 加载预训练模型
model = YOLO(r'E:\pythonProject\Microalgae_Identification_YOLOv11\runs\segment\train3\weights\best.pt')
...
def merge_overlapping_masks(results, iou_threshold=0.5, class_agnostic=False, area_threshold=10):
    ...
    for group in merged_groups:
        ...
        # 计算合并后的置信度（加权平均）
        merged_conf = np.average(group['confs'], weights=group['confs'])
...
def process_folder(folder_path):
    ...
    # 重新创建results文件夹（仅当有需要保存的结果时才实际使用）
    ...
    # 执行匹配
    match_1 = re.search(pattern, folder_path)
    if match_1:
        excel_path = os.path.join(root_output, f"CH{int(match_1.group(1))}_CB{int(match_1.group(2))}.xlsx")
    else:
        excel_path = os.path.join(root_output, f"test.xlsx")
    ...
    with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
        ...
        def extract_number(filename):
            ...
            # 匹配文件名中的数字（针对CH1_CB1_H1.tif这样的格式）
            match = re.search(r'_H(\d+).tif', basename)
            if match:
        ...
        for image_path in tqdm(images):
            try:
                ...
                # 目标检测
                results = model(image_path, max_det=1000, task='segment')
                target_count = 0
                total_area = 0
                target_details = []
                ...
                if results and len(results) > 0 and results[0].boxes is not None and len(results[0].boxes) > 0:
                    ...
                    if len(results[0].masks) < 30:
                        ...
                        print(f"识别目标超过30，共有{len(results[0].masks)}个掩膜，不执行合并掩膜操作")
                        result = results[0]
                ...
                # 仅当检测到目标时才生成并保存结果图，且创建对应Excel页签
                if target_count > 0:
                    # 标记有有效图片
                    ...
                    # 未检测到目标：不生成结果图，不创建Excel页签，仅在汇总表记录
                    summary_data.append({
                        '图片名称': img_name,
                        '目标数量': 0,
                        '总面积(μm²)': "0.00"
...
# # 指定要处理的文件夹路径
# folder_path = r'F:\Microalgae_Photoes\text_photoes\CH1\IMG001x002'
