...
def build_predict_attempts():
    if torch.cuda.is_available():
        return [
            {"device": 0, "imgsz": 1024, "max_det": 300, "half": True},
            {"device": 0, "imgsz": 768, "max_det": 200, "half": True},
            {"device": 0, "imgsz": 640, "max_det": 100, "half": False},
            {"device": "cpu", "imgsz": 640, "max_det": 100, "half": False},
        ]
    return [
        {"device": "cpu", "imgsz": 640, "max_det": 100, "half": False},
    ]
...
def predict_with_retry(model, source, dense_mode=False):
    errors = []
    ...
    for idx, cfg in enumerate(attempts, start=1):
        try:
            ...
            if dense_mode:
                kwargs.update({
                    "conf": 0.1,       # 降低置信度阈值，不放过弱信号
                    "iou": 0.6,         # NMS IoU 阈值，保留更多重叠检测
                    "augment": True,    # TTA 多尺度推理取平均
                    "max_det": max(cfg["max_det"], 500),
                })
            print(
                f"  尝试 {idx}/{len(attempts)}: "
                f"device={cfg['device']}, imgsz={cfg['imgsz']}, "
                f"max_det={kwargs['max_det']}, half={cfg['half']}"
                f"{', conf=0.1, iou=0.6, augment' if dense_mode else ''}"
            )
...
def preprocess_for_dense(image_bgr):
    """CLAHE+锐化增强细胞边界对比度，改善黏连场景下的检测效果"""
    ...
    with pd.ExcelWriter(excel_path, engine="openpyxl") as writer:
        ...
        for image_path in tqdm(images):
            ...
            try:
                ...
                # 像素→微米转换
                pixel_to_um = {
                    "width": ACTUAL_WIDTH_UM / original_img.shape[1],
                    "height": ACTUAL_HEIGHT_UM / original_img.shape[0],
                }
                avg_conversion = (pixel_to_um["width"] + pixel_to_um["height"]) / 2

                # CLAHE+锐化增强 → 送入YOLO推理（改善黏连场景检测）
                enhanced_img = preprocess_for_dense(original_img)
                pred_results, cfg_used, pred_error = predict_with_retry(model, enhanced_img, dense_mode=True)
                if pred_results is None or len(pred_results) == 0:
                    raise RuntimeError(f"推理失败: {pred_error}")

                detection = pred_results[0]
                # 恢复原图，使YOLO标注在原图上绘制
                detection.orig_img = original_img
