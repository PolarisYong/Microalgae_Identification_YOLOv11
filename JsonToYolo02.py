...
def convert_labelme_to_yolo(json_path, img_dir, output_dir, class_dict, save_masks=False):
    ...
    if yolo_lines:
        # 提取图像纯文件名（去掉路径和扩展名）
        img_basename = os.path.basename(img_filename)  # 拿到类似 "IMG001x001.jpg"
        txt_basename = os.path.splitext(img_basename)[0] + '.txt'  # 得到 "IMG001x001.txt"

        txt_path = os.path.join(output_dir, "labels", txt_basename)  # 正确拼接路径
