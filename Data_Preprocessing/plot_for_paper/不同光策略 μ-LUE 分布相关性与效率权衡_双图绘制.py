...
# 新增：全局设置坐标轴刻度数字大小
plt.rcParams['xtick.labelsize'] = 18  # x轴刻度数字大小
plt.rcParams['ytick.labelsize'] = 18  # y轴刻度数字大小
...
confidence_ellipse(
    ...
    edgecolor='#666666', linestyle=':', linewidth=1.6,  # 深灰色边界
...
# 3.6 子图a细节优化（去掉网格线）
ax1.set_xlabel(r'Average specific growth rate of biomass ($\bar{\mu}$, h$^{-1}$)', fontsize=18, fontweight='bold')
ax1.set_ylabel('Light use efficiency (LUE, %)', fontsize=18, fontweight='bold')
ax1.set_title('(a) μ-LUE Distribution and Correlation', fontsize=18, fontweight='bold')
ax1.set_xlim(-0.01, 0.20)
ax1.set_ylim(-1, 20)
ax1.set_xticks(np.arange(0, 0.20, 0.04))
ax1.set_yticks(np.arange(0, 20, 4))
ax1.grid(False)  # 去掉网格线
ax1.legend(loc='upper left', framealpha=0.9, bbox_to_anchor=(0.01, 0.99), prop=tnr_font)
# 3.7 添加图注说明（虚线表示回归线）
...
# 4.1 【修改1】去掉粉色高效区色块（删除原fill_between代码）
...
# 右下角新图例（仅包含3条效率线，按要求格式显示）
# line_handles = line_low + line_balance + line_high
# line_labels = [h.get_label() for h in line_handles]
# # 添加图例标题（Isopleths of efficiency ratio）
# ax2.legend(handles=line_handles, labels=line_labels,
#            loc='lower right', fontsize=10, framealpha=0.9, bbox_to_anchor=(1.005, -0.005),
#            title=r'Isopleths of efficiency ratio ($\eta_r$ = LUE / $\bar{\mu}$)', title_fontsize=9)
# 4.5 子图b细节优化（保持其他设置不变）
ax2.set_xlabel(r'Average specific growth rate of biomass ($\bar{\mu}$, h$^{-1}$)', fontsize=18, fontweight='bold')
ax2.set_ylabel('Light use efficiency (LUE, %)', fontsize=18, fontweight='bold')
ax2.set_title('(b) μ-LUE Efficiency Trade-off & High-Efficiency Region', fontsize=18, fontweight='bold')
# 统一坐标轴范围（与子图a完全一致）
...
plt.close()
# 运行完成提示
print("Final version of the fully optimized dual plot has been generated!")
